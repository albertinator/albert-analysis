 ...
 * A ledger file for the year: either an `.xlsx` file (legacy) or a `.webloc` link to a Google Sheet (the new way)
  * If it's a `.webloc` link to a Google Sheet, you should be able to access this sheet (via Google Sheets MCP) by referencing the Spreadsheet ID in the URL structure that's contained inside this file. For the most part, the receipts in this folder should have a corresponding entry in the Google Sheet under the relevant tab.
